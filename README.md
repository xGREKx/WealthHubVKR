...
- ✅ **Кабинет предпринимателя — все статусы**: страница «Мои проекты» теперь показывает черновики, проекты на модерации, **одобренные**, отклонённые и закрытые. Загрузка через `GET /api/projects/?mine=1` — отдельный server-side фильтр для текущего пользователя.
- ✅ **Список инвесторов проекта**: предприниматель видит, кто из инвесторов сколько вложил, какую долю это составляет и когда была первая инвестиция. Endpoint `GET /api/projects/<id>/investors/` агрегирует несколько вкладов одного инвестора в одну строку и сортирует по убыванию суммы. Доступ только владельцу проекта и администратору.
